...
static const QMap<QString, QVariant> defaults {
  ...
  {keys::USE_OCC_NOT_TONTO, false}, {keys::GAUSSIAN_EXECUTABLE, ""},
      {keys::GAUSSIAN_MDEF, ""}, {keys::GAUSSIAN_PDEF, ""},

      {keys::NWCHEM_EXECUTABLE, ""}, {keys::PSI4_EXECUTABLE, ""},
      {keys::ORCA_EXECUTABLE, ""}, {keys::ORCA_NTHREADS, ""},
      {keys::XTB_EXECUTABLE, ""}, {keys::OCC_EXECUTABLE, ""},
      {keys::OCC_BASIS_DIRECTORY, ""}, {keys::OCC_NTHREADS, 1},
// Tonto
#if defined(Q_OS_LINUX)
      {keys::TONTO_PATH, GLOBAL_TONTO_PATH},
#endif
      {keys::TONTO_EXECUTABLE, GLOBAL_TONTO_EXECUTABLE},
      {keys::TONTO_BASIS_DIRECTORY, GLOBAL_TONTO_BASIS_DIRECTORY},
      {keys::DISABLE_XH_NORMALIZATION, false}, {keys::CH_BOND_LENGTH, 1.083f},
      {keys::NH_BOND_LENGTH, 1.009f}, {keys::OH_BOND_LENGTH, 0.983f},
      {keys::BH_BOND_LENGTH, 1.180f},
      {keys::USE_CLEMENTI, false}, // Thakkar is now the default
      {keys::PREFERRED_WAVEFUNCTION_SOURCE, "tonto"},
      // CrystalExplorer settings
      {keys::EXECUTABLE, "CrystalExplorer"},
      {keys::ELEMENTDATA_FILE, GLOBAL_ELEMENTDATA_FILE},
      {keys::USE_JMOL_COLORS, false}, {keys::DELETE_WORKING_FILES, true},
      {keys::AUTOLOAD_LAST_FILE, false}, {keys::BACKGROUND_COLOR, "white"},
      ...
      {keys::SURFACE_STENCIL_BUFFER_SIZE, 8}, {keys::SURFACE_NUMBER_SAMPLES, 4},
      ...
      {keys::LIGHT_AMBIENT_INTENSITY, 2.0f}, {keys::LIGHT_INTENSITY_1, 10.0f},
      {keys::LIGHT_INTENSITY_2, 10.0f}, {keys::LIGHT_INTENSITY_3, 10.0f},
      {keys::LIGHT_INTENSITY_4, 10.0f},
      {keys::LIGHT_ATTENUATION_MINIMUM, 0.2f},
      {keys::LIGHT_ATTENUATION_MAXIMUM, 10.0f}, {keys::LIGHTING_EXPOSURE, 1.0f},
      {keys::LIGHTING_TONEMAP, 1}, {keys::MATERIAL, 2},
      {keys::TEXT_OUTLINE, 0.15f}, {keys::TEXT_BUFFER, 0.15f},
      {keys::TEXT_SMOOTHING, 0.15f},
      {keys::TEXT_COLOR, QVariant::fromValue(Qt::black)},
      {keys::TEXT_OUTLINE_COLOR, QVariant::fromValue(Qt::gray)},
      ...
      // molecule lighting ...
      {keys::MATERIAL_ROUGHNESS, 0.25f}, {keys::MATERIAL_METALLIC, 0.05f},
