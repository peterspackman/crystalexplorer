...
void addLineToLineRenderer(LineRenderer &r, const QVector3D &pointA,
                           const QVector3D &pointB, float lineWidth,
                           const QColor &color, const QVector3D &idA = {},
                           bool selected = false);
void addDashedLineToLineRenderer(LineRenderer &r, const QVector3D &pointA,
                                 const QVector3D &pointB, float lineWidth,
                                 const QColor &color, float dashLength = 0.1,
                                 float dashSpace = 0.1);
void addCurvedLineBetweenVectors(CircleRenderer &r, QVector3D v0, QVector3D v1,
...
void addCylinderToCylinderRenderer(CylinderRenderer *r, const QVector3D &pointA,
                                   ...
                                   bool selectedA = false,
                                   bool selectedB = false);
void addEllipsoidToEllipsoidRenderer(
    EllipsoidRenderer *, const QVector3D &position, const QMatrix3x3 &transform,
    const QColor &color, const QVector3D &id = {}, bool selected = false);
...
makePointCloudVertices(const Mesh &, ColorSettings settings = {});
