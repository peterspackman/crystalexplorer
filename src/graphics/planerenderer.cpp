...
PlaneRenderer::PlaneRenderer()
  ...
  m_program->setAttributeBuffer(2, GL_FLOAT, PlaneInstanceData::originOffset(),
                                PlaneInstanceData::OriginTupleSize,
                                PlaneInstanceData::stride());
  ...
  m_program->setAttributeBuffer(3, GL_FLOAT, PlaneInstanceData::axisAOffset(),
                                PlaneInstanceData::AxisATupleSize,
                                PlaneInstanceData::stride());
  ...
  m_program->setAttributeBuffer(4, GL_FLOAT, PlaneInstanceData::axisBOffset(),
                                PlaneInstanceData::AxisBTupleSize,
                                PlaneInstanceData::stride());
  ...
  m_program->setAttributeBuffer(5, GL_FLOAT, PlaneInstanceData::colorOffset(),
                                PlaneInstanceData::ColorTupleSize,
                                PlaneInstanceData::stride());
  ...
  m_program->setAttributeBuffer(
      6, GL_FLOAT, PlaneInstanceData::gridParamsOffset(),
  ...
  m_program->setAttributeBuffer(7, GL_FLOAT, PlaneInstanceData::boundsAOffset(),
                                PlaneInstanceData::BoundsATupleSize,
                                PlaneInstanceData::stride());
  ...
  m_program->setAttributeBuffer(8, GL_FLOAT, PlaneInstanceData::boundsBOffset(),
                                PlaneInstanceData::BoundsBTupleSize,
                                PlaneInstanceData::stride());
...
PlaneRenderer::~PlaneRenderer() { delete m_texture; }
...
void PlaneRenderer::initializeGeometry() {
  ...
  m_vertices = {
      PlaneVertex(QVector3D(0.0f, 0.0f, 0.0f),
                  QVector2D(0.0f, 0.0f)), // bottom-left
      PlaneVertex(QVector3D(1.0f, 0.0f, 0.0f),
                  QVector2D(1.0f, 0.0f)), // bottom-right
      PlaneVertex(QVector3D(1.0f, 1.0f, 0.0f),
                  QVector2D(1.0f, 1.0f)), // top-right
      PlaneVertex(QVector3D(0.0f, 1.0f, 0.0f),
...
PlaneInstanceData
PlaneRenderer::createInstanceData(Plane *plane, PlaneInstance *instance) const {
  ...
  QVector4D gridParams(
      plane->showGrid() ? 1.0f : 0.0f, static_cast<float>(plane->gridSpacing()),
      plane->showAxes() ? 1.0f : 0.0f, plane->showBounds() ? 1.0f : 0.0f);
  ...
  return PlaneInstanceData(origin, axisA, axisB, color, gridParams, boundsAVec,
                           boundsBVec);
}

void PlaneRenderer::beginUpdates() { m_updatesDisabled = true; }
...
void PlaneRenderer::updateBuffers() { updateInstanceBuffer(); }
...
void PlaneRenderer::draw() {
  qDebug() << "PlaneRenderer::draw() called with" << m_instanceData.size()
           << "instances, indices:" << m_numberOfIndices;
  ...
  qDebug() << "PlaneRenderer: Drew" << m_instanceData.size()
           << "plane instances";
}
