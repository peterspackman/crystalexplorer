...
ElasticTensorResults::ElasticTensorResults(QObject *parent)
    : QObject(parent), m_name("Elastic Tensor"), m_elasticMatrix(occ::Mat6::Zero()) {
...
QString ElasticTensorResults::name() const {
    return m_name;
}
...
QString ElasticTensorResults::description() const {
...
double ElasticTensorResults::shearModulus(const occ::Vec3 &direction, double angle) const {
    updateTensor();
    if (!m_tensor) {
        qDebug() << "Error: Elastic tensor not initialized";
        return 0.0;
...
double ElasticTensorResults::linearCompressibility(const occ::Vec3 &direction) const {
    updateTensor();
    if (!m_tensor) {
        qDebug() << "Error: Elastic tensor not initialized";
        return 0.0;
...
double ElasticTensorResults::poissonRatio(const occ::Vec3 &direction, double angle) const {
    updateTensor();
    if (!m_tensor) {
        qDebug() << "Error: Elastic tensor not initialized";
        return 0.0;
...
double ElasticTensorResults::averageBulkModulus(occ::core::ElasticTensor::AveragingScheme scheme) const {
    updateTensor();
    if (!m_tensor) {
        qDebug() << "Error: Elastic tensor not initialized";
        return 0.0;
...
double ElasticTensorResults::averageShearModulus(occ::core::ElasticTensor::AveragingScheme scheme) const {
    updateTensor();
    if (!m_tensor) {
        qDebug() << "Error: Elastic tensor not initialized";
        return 0.0;
...
double ElasticTensorResults::averageYoungsModulus(occ::core::ElasticTensor::AveragingScheme scheme) const {
    updateTensor();
    if (!m_tensor) {
        qDebug() << "Error: Elastic tensor not initialized";
        return 0.0;
...
double ElasticTensorResults::averagePoissonRatio(occ::core::ElasticTensor::AveragingScheme scheme) const {
    updateTensor();
    if (!m_tensor) {
        qDebug() << "Error: Elastic tensor not initialized";
        return 0.0;
...
Mesh *ElasticTensorResults::createPropertyMesh(PropertyType property, 
                                              int subdivisions, double radius) const {
    // Validate inputs
    ...
    try {
        for (int i = 0; i < unitVertices.cols(); ++i) {
            ...
            if (!std::isfinite(compress)) compress = 0.0;
    ...
    if (maxValue > 1e-10) {
        ...
        qDebug() << "Scaled mesh with max property value:" << maxValue << "radius:" << radius;
    } else {
    ...
    Mesh *mesh = nullptr;
    try {
        mesh = new Mesh(scaledVertices, faces, const_cast<ElasticTensorResults*>(this));
    } catch (const std::exception& e) {
    ...
    try {
        ...
        mesh->setVertexProperty("None", Eigen::VectorXf::Zero(scaledVertices.cols()));
        ...
        if (auto* structure = qobject_cast<ChemicalStructure*>(this->parent())) {
            ...
            atomsOutside.erase(
                std::remove_if(atomsOutside.begin(), atomsOutside.end(),
                    [&atomsInside](const GenericAtomIndex& idx) {
                        return std::find(atomsInside.begin(), atomsInside.end(), idx) != atomsInside.end();
                    }), 
                atomsOutside.end());
            ...
            qDebug() << "Calculated atoms inside elastic tensor mesh:" << atomsInside.size() << "inside," << atomsOutside.size() << "outside";
        }
        ...
        for (const QString& propName : propNames) {
            ...
            if (propValues.size() != mesh->numberOfVertices()) {
                qDebug() << "Error: Property" << propName << "size mismatch. Expected:" << mesh->numberOfVertices() << "Got:" << propValues.size();
                delete mesh;
                return nullptr;
            }
        }
        
        qDebug() << "Created scaled property mesh - vertices:" << mesh->numberOfVertices() << "faces:" << mesh->numberOfFaces() << "max value:" << maxValue;
...
nlohmann::json ElasticTensorResults::toJson() const {
    ...
    j["averageProperties"] = {
        {"bulkModulus", averageBulkModulus()},
...
ElasticTensorCollection::ElasticTensorCollection(QObject *parent) : QObject(parent) {
}
...
int ElasticTensorCollection::count() const {
...
ElasticTensorResults *ElasticTensorCollection::findByName(const QString &name) const {
...
bool ElasticTensorCollection::fromJson(const nlohmann::json &json) {
    try {
        ...
        qDebug() << "Error loading elastic tensor collection from JSON:" << e.what();
        return false;
    }
}