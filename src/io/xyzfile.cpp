...
XYZFile::XYZFile(const std::vector<QString> &atomSymbols, Eigen::Ref<const occ::Mat3N> atomPositions) {
...
void XYZFile::setElements(Eigen::Ref<const occ::IVec> nums) {
	...
	m_atomSymbols[i] = QString::fromStdString(occ::core::Element(nums(i)).symbol());
    }
}
...
const QString &XYZFile::getComment() const {
    return m_comment;
}

void XYZFile::setComment(const QString &comment) {
    m_comment = comment;
}
