...
Mesh *read_ply_file(const QString &filepath, bool preload_into_memory = true) {
  ...
  try {
    ...
    if (!file_stream || file_stream->fail())
      throw std::runtime_error("file_stream failed to open " + filepathStd);
    ...
    // Assuming the existence of a PlyFile class or similar to handle the
    // parsing
    auto vertices =
    ...
    std::vector<std::string> property_names = {"dnorm",
                                               "di_idx",
                                               "de_idx",
                                               "di",
                                               "di_norm",
                                               ...
                                               "k1",
                                               "k2"};
    ankerl::unordered_dense::map<std::string, std::shared_ptr<tinyply::PlyData>>
        properties;
    for (const auto &prop_name : property_names) {
      std::shared_ptr<tinyply::PlyData> prop;
      try {
        prop = file.request_properties_from_element("vertex", {prop_name});
    ...
    faceMatrix = Eigen::Map<const Eigen::Matrix<uint32_t, 3, Eigen::Dynamic>>(
                     reinterpret_cast<const uint32_t *>(faces->buffer.get()), 3,
                     faces->count)
                     .cast<int>();

    normalMatrix = Eigen::Map<const Eigen::Matrix3Xf>(
                       reinterpret_cast<const float *>(normals->buffer.get()),
                       3, normals->count)
                       .cast<double>();
    ...
    for (const auto &[prop_name, prop] : properties) {
      qDebug() << QString::fromStdString(prop_name) << "read" << prop->count
               << "property values";
      switch (prop->t) {
...
Mesh *loadMesh(const QString &filename) { return read_ply_file(filename); }
