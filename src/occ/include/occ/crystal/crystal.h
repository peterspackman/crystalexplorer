...
public:
   ...
   */
  std::vector<CrystalAtomRegion>
  unit_cell_atom_surroundings(double radius) const;

  /**
   * \brief Returns the atoms within a certain radius of the specified positions
   *
   * This method can be used to find the neighbouring atoms for a set of
   * provided positions.
   *
   * \param positions the (fractional) positions of atoms surrounding the
   * provided positions.
   *
   * \param radius The maximum distance (Angstroms) from the
   * specified atom that an atom can be in order to be included in the result.
   * (default = 6.0)
   *
   * \param tolerance The distance that is considered 'too close' to one of the
   * given positions
   *
   * \return A `CrystalAtomRegion` object containing the atoms within the
   * specified radius of the specified atom. The `CrystalAtomRegion` class
   ...
   */
  std::vector<CrystalAtomRegion>
  atoms_surrounding_points(const Mat3N &frac_pos, double radius,
                           double tolerance = 1e-1) const;
