...
float intersectCorner(out vec3 cameraPos, out vec3 cameraNormal,
                      out float alpha, vec2 mapping) {
  ...
  vec2 intersection =
      sphIntersect(vec3(0.0), rayDirection, v_cameraSpherePos, v_radius);

  alpha = 1.0;
  if (intersection.x < 0.0) {
    // More direct anti-aliasing
    float dist_to_center = length(v_mapping);
    alpha = 1.0 -
            smoothstep(1.0 - fwidth(dist_to_center) * 1.5, 1.0, dist_to_center);

    if (alpha < 0.01) {
      discard; // Discard fragments that are clearly outside
    }
    // When anti-aliasing, don't project onto the sphere surface.
    // Instead, let the background show through by pushing the depth to the
    // back.
    cameraPos = vec3(cameraPlanePos.xy, v_cameraSpherePos.z + v_radius);
    cameraNormal = normalize(cameraPos - v_cameraSpherePos);

    return -1.0; // Indicate that this is an edge pixel
  }
...
void main() {
  ...
  outputColor = calculateShading(u_renderMode, v_color.xyz, u_cameraPosVec,
                                 worldSpacePos.xyz, worldSpaceNormal.xyz, alpha,
                                 v_object_id);
